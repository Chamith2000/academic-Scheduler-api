...
const InstructorDashboardPage = () => {
    ...
    // Load auth from localStorage if not present (fallback)
    useEffect(() => {
        if (!auth) {
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); handleAddPreference(); }}>
                        ...
                        <div className="mb-6">
                            ...
                            <select
                                ...
                                className={`w-full px-4 py-2 border ${formErrors.timeslotSelect ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-blue-500"} rounded-md focus:outline-none focus:ring-2`}
                                required
                            >
                            ...
                            {formErrors.timeslotSelect && <p className="text-red-500 text-xs mt-1">{formErrors.timeslotSelect}</p>}
                        </div>
                ...
                <div className="main-content p-6 bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                            <QuickStatCard icon={HiAcademicCap} title="Total Courses" value={stats.totalCourses} change={20} positive={true} />
                            <QuickStatCard icon={HiUserGroup} title="Active Students" value={stats.activeStudents} change={15} positive={true} />
                            <QuickStatCard icon={HiDocumentText} title="Pending Assignments" value={stats.assignmentsPending} change={-5} positive={false} />
                            <QuickStatCard icon={HiCalendar} title="Upcoming Classes" value={stats.upcomingClasses} change={0} positive={true} />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md">
                                ...
                                {recentActivities.map((activity, index) => (
                                    <RecentActivityItem key={index} course={activity.course} action={activity.action} time={activity.time} />
                                ))}
                            </div>
                            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md">
                                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-4">Upcoming Events</h2>
                                <div className="space-y-4">
                                    <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border-l-4 border-blue-500">
                                        <h3 className="font-semibold text-blue-800 dark:text-blue-300">Machine Learning Lecture</h3>
                                        <p className="text-sm text-gray-600 dark:text-gray-400">Today, 2:00 PM - Room 305</p>
                                    </div>
                                    <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border-l-4 border-green-500">
                                        <h3 className="font-semibold text-green-800 dark:text-green-300">Web Development Workshop</h3>
                                        <p className="text-sm text-gray-600 dark:text-gray-400">Tomorrow, 10:00 AM - Online</p>
                                    </div>
                                    <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border-l-4 border-yellow-500">
                                        <h3 className="font-semibold text-yellow-800 dark:text-yellow-300">Database Systems Quiz</h3>
                                        <p className="text-sm text-gray-600 dark:text-gray-400">Next Week, 3:00 PM - Lab 202</p>
                                    </div>
                                </div>
                            </div>
                        </div>
